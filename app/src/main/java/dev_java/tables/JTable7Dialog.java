...
public class JTable7Dialog extends JDialog implements ActionListener {
 ...
 public JTable7Dialog(DeptTable7 deptTable7) {
  this.deptTable7 = deptTable7;// 부모쪽의 메소드 call
  initDisplay();
 }
 ...
 // actionPerformed에서 이벤트(입력,수정,상세보기) 발생되면 호출됨
 ...
 public void setValue(DeptVO pdVO) {
  ...
  // 상세조회 , 수정시는 배열로 받은 값으로 셋팅
  // 부모창에서 set메소드 호출시 파라미터로 넘겨준 값으로 초기화 할것.
  else {
 ...
 public void actionPerformed(ActionEvent e) {
  ...
  if (obj == jbtn_save) {
   ...
   // 다이얼로그 화면에서 저장버튼 하나로 어떨때는 입력처리를 어떤경우는 수정처리를 한다.
   ...
   // 입력해준다(내안에 수정하고자하는 로우의 DeptVO가 있고 그 안에는 deptno,dname,loc가 있다.)
   if (pdVO != null) {
    ...
    for (int i = 0; i < DeptTable7.vdata.size(); i++) {
     ...
     // 벡터에서 꺼낸 부서번호와 부모창(DeptTable7)에서 받아온 DeptVO의 부서번호가 같은지 비교해야함
